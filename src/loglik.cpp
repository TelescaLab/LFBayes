...
arma::mat loglik(arma::vec y, arma::mat X, arma::mat Bs, arma::mat Bt,
  ...
  arma::uword num_not_missing = ntot - arma::uword(arma::sum(missing_indicator));
  arma::vec fit(ntot);
  arma::mat logliks(nsim, num_not_missing);
  
  for(arma::uword i = 0; i < nsim; i++){
    for(arma::uword j = 0; j < nsub; j++){
      fit.subvec(ns * nt * j, ns * nt * (j + 1) - 1) = arma::vectorise(Bt * Theta(i).slice(j) * Bs.t());
    }
      logliks.row(i) = arma::trans(arma::log_normpdf(y, fit(obs), arma::vec(obs.n_elem).fill(1.0 / Varphi(0)(i))));
  }
  return(logliks);
}