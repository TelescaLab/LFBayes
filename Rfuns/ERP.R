...
nchain <- 4
burnin <- 1000
ERPFebruaryMCMC1 <- mcmcWeakChains(y, missing, X, bs1, bt1, q1, q2, iter, nchain, burnin, nchain)
...
ERPFebruaryeigenMCMC1 <- eigenLFChains(bs1, bt1, ERPFebruaryMCMC1, 3, iter, 4995, nchain)
save(ERPFebruaryeigenMCMC1, file = "ERPFebruaryeigenMCMC1.RData")

A <- calculate_DIC_Missing(y, X, observed, ERPFebruaryMCMC1, bs1, bt1, 4990, 5)
B <- calculate_BIC_Missing(y, X, observed, ERPFebruaryMCMC1, bs1, bt1, 4990, 5)
...
X <- rep(1,length(subjects))
dim(X) <- c(length(subjects),1)
observed <- lapply(1:length(subjects), function(i) setdiff(1:56, missing[[i]]))
iter <- 5000
nchain <- 4
burnin <- 1000
ERPFebruaryMCMC2 <- mcmcWeakChains(y, missing, X, bs1, bt1, q1, q2, iter, nchain, burnin, nchain)
...
nchain <- 4
burnin <- 1000
ERPFebruaryMCMC3 <- mcmcWeakChains(y, missing, X, bs1, bt1, q1, q2, iter, nchain, burnin, nchain)
