...
void updateGammaSig(arma::cube &eta, arma::mat &Lambda, arma::vec Deltastar,
  ...
  for(arma::uword i = 0; i < Phistar.n_rows; i++){
    ...
    for( subject = 0; subject < eta.n_slices; subject++){
      ...
      muMat.row(i) = muMat.row(i) + theta.slice(subject).col(i).t() *
...
arma::mat updatePhistar(arma::mat &Gamma, arma::vec deltastar){
  ...
  for(arma::uword i = 0; i < Phistar.n_cols; ++i){
    for(arma::uword j = 0; j < Phistar.n_rows; ++j){
      Phistar(j,i) = R::rgamma((v + 1.0) / 2.0, 1.0 / (v / 2.0 + 1.0/2.0 *
        tauH(i) * ::pow(Gamma(j,i), 2.0)));
    }
  }
  return Phistar;
}
...
arma::vec updateDelta(arma::mat &Lambda, arma::mat &Phi,
  ...
  arma::vec tempD = tauH;
  tempD(0) = 1.0;
  Delta(0) = R::rgamma(a1 + p*k / 2.0, 1.0 / (1.0 + 1.0 / 2.0 *
    arma::dot(tempD, colsums)));
  ...
  for(arma::uword i = 1; i < Delta.n_elem; ++i){
    ...
    Delta(i) = gam_trunc_left(a2 + p*(k-i)/2.0, 1.0 / (1.0 + 1.0/2.0 *
      tempSum), 1.0);
  }
  return Delta;
}
...
void updateLambdaSig(arma::cube &eta, arma::mat &Gamma, arma::vec Delta,
                     arma::mat &Phi, arma::mat Sigma,
                     arma::cube &theta, arma::mat &Lambda){
...
arma::mat updatePhi(arma::mat &Lambda, arma::vec delta){
  ...
  for(arma::uword i = 0; i < Phi.n_cols; ++i){
    for(arma::uword j = 0; j < Phi.n_rows; ++j){
      Phi(j,i) = R::rgamma((v + 1.0) / 2.0, 1.0 / (v / 2.0 + 
        1.0 / 2.0 * tauH(i) * ::pow(Lambda(j,i), 2.0)));
    }
  }
  return Phi;
}
  ...
  for(arma::uword i=0; i < p2; i++){
    for(arma::uword j=0; j < p1;j++){
      for(arma::uword subject=0; subject<Theta.n_slices;subject++){
        ztz = ztz + 
          ::pow(arma::as_scalar(Theta.slice(subject).col(i).row(j) - 
          Lambda.row(j) * Eta.slice(subject) * arma::trans(Gamma.row(i))),2);
      }
      Sigma(j,i) = R::rgamma(asig + double(Theta.n_slices)/2.0,
            1.0 / (bsig + 1.0/2.0 * ztz));
      ztz = 0;
    }
  }
  return(Sigma);
...
double updateVarphi(arma::cube &theta, arma::mat &splineS,
                    arma::mat &splineT, arma::mat &y){
  double aVarphi = .0001;
  double bVarphi = .0001;
...
void updateMissing(arma::mat &y, arma::field<arma::vec> &missing,
                   arma::cube &theta, double Varphi,
                   arma::mat &splineS, arma::mat &splineT, int n){
  for(int i = 0; i < n; i++){
    arma::vec currentMissing = missing(i);
    for(arma::uword m = 0; m < currentMissing.n_elem; m++){
      y.col(i).subvec(splineT.n_rows * (currentMissing(m)-1), splineT.n_rows * 
        (currentMissing(m)-1) + splineT.n_rows - 1) = arma::vectorise(splineT * 
        theta.slice(i) * arma::trans(splineS.row(currentMissing(m)-1))) +
